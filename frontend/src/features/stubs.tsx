...
import { useConsolidate } from '../shared/hooks/useConsolidate';
import { useSearch } from '../shared/hooks/useSearch';
import { useUsers } from '../shared/hooks/useUsers';
import type { UserListItem } from '../shared/hooks/useUsers';
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-6">
        ...
        <div
          ...
          onDrop={handleDrop}
          className={`border-2 border-dashed rounded-xl p-10 text-center transition-all duration-200 relative group cursor-pointer ${dragOver ? 'border-cyan-500 bg-cyan-50/20' : 'border-slate-300 hover:border-slate-400 bg-slate-50/50'
            }`}
                    ...
                    <td className="p-4 min-w-[150px]">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-300 ${job.status === 'failed' ? 'bg-rose-500' :
...
function useForceLayout(
  ...
  React.useEffect(() => {
    ...
    // Defer heavy computation off the main thread so the browser can paint first
    const timerId = setTimeout(() => {
      ...
      const iterations = 80; // Reduced from 320 – avoids blocking the main thread
      const padding = 112;
    ...
    }, 30); // 30ms gives the browser one frame to paint the loading state first

    return () => clearTimeout(timerId);
  }, [nodes, edges, width, height]);

  return { positions, isComputing };
}
...
export const GraphExplorerPage: React.FC = () => {
  ...
  const { positions, isComputing: isComputingLayout } = useForceLayout(nodes, edges, SVG_W, SVG_H);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
        ...
        <div className="lg:col-span-2 bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden relative shadow-xl" style={{ minHeight: 560 }}>
          ...
          {(isFetchingGraph || isComputingLayout) ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 z-10">
              ...
              <span className="text-xs font-bold tracking-wide">
                {isFetchingGraph ? 'Traversing Neo4j network paths...' : 'Computing graph layout...'}
              </span>
            </div>
          ) : nodes.length === 0 ? (
              ...
              <tr key={u.id} className="hover:bg-slate-50/50">
                ...
                <td className="p-4 text-right space-x-2 shrink-0">
                  ...
                  <button
                    ...
                    className={`px-2 py-1 text-[10px] font-bold rounded-lg border transition-colors ${u.isActive
